...
function checkPrereqs() {
  ...
  if [[ $? -ne 0 || ! -d "../config" ]]; then
    ...
    echo
    read -p "Shall we proceed with the installation of docker and binaries: Press [y/n]?`echo $'\n> '`" -n 1 -r
    if [[ $REPLY =~ ^[Yy]$ ]]; then
      cd ..
      bash ./test-network/installer.sh -s -- 2.2.0 1.4.8
      cd -
      echo "Binaries and docker images installed"
    else
